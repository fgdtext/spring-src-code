	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				/*
					怎么找？？？
					1. 扫描 @Controller 注解（spring 已经做了，已经在容器中了）
					2. 拿到所有加了 @Controller 注解的 类。MVC（起作用，在接收请求之前就做了）
					3. 遍历类里边所有的方法
					4. 判断是否加了 @RequestMapping 注解
					5. 把@RequestMapping 注解 的value(URI) 作为 map 集合 的key 给 put 进去。value 就是method 对象。

					6. 根据用户发送的请求， 拿到请求中的 URI
					7. 使用请求的 uri 作为key 在map中找相应的value .
				...
				// 获取适配器， 适配器： 兼容当前 handler 的调用方式。负责调用handler ,所以要支持当前handler。
				// 适配器有三种， 分别对应了 Controller的三种实现（一种注解，两种实现接口。）
				...
				//  执行前置拦截器， 若返回true ,则通过， 返回false 则return，被拦截了。
				...
				// ModelAndView : mv 是返回的结果体，还要对mv 进行解析
