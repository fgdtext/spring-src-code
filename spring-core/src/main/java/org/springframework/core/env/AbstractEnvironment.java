...
public abstract class AbstractEnvironment implements ConfigurableEnvironment {
	...
	public AbstractEnvironment() {
		//  设置属性值。
		/*
			propertySources ： 内有一个propertySourceList：2个。 web项目多3个集合
			存放StandardEnvironment中systemEnvironment属性集合和systemProperties属性集合

			而propertySources 属于Environment： 所以要获取属性找Environment的propertySources即可。
			根据属性不同分类找propertySources的propertySourceList
